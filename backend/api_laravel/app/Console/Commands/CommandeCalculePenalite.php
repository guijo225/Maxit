...
class CommandeCalculePenalite extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:commande-calcule-penalite';
    ...
    {
        \Log::info('Début du calcul des pénalités');
        ...
        foreach ($tontines as $tontine) {
            ...
            $tour = Tour::where('id_tontine', $id_tontine)->latest()->first();
            ...
            if ($tour) {
                ...
                if (!empty($participantIds)) {
                    ...
                    foreach ($participantsPenalises as $id_penalises) {
                        ...
                        $participant = Participant::where('id_participant', $id_penalises)->with('utilisateur')->get();
                        $id_utilisateur = $participant->id_utilisateur;
                        $nom_utilisateur = $participant->utilisateur->prenom;
                        if (!$penalites) {
                            $mnt = $montant_a_cotise * self::POURCENTAGE_PENALITE;
                            $penalite = createPenalites($mnt, $id_tour, $id_penalises);
                            $description = "Une pénalité à été infligé à " . $nom_utilisateur . " pour retard de paiement dans la tontine " . $nom_tontine . ".";
                            $notif = createNotification($id_utilisateur, $description, $id_tontine);
                            $penalitesTraitees++;
                        }else {
                            $dateDernierCalcul = Carbon::parse($penalites->updated_at);
                            $joursPasses = $dateDernierCalcul->diffInDays(now());
                            if ($joursPasses > 0) {
                                $ajoutMntPenalite = $montant_a_cotise * self::POURCENTAGE_PENALITE;
                                ...
                                $description = "La pénalité infligé à " . $nom_utilisateur . " pour retard de paiement dans la tontine " . $nom_tontine . " a augmenté pour non paiement";
                                $notif = createNotification($id_utilisateur, $description, $id_tontine);
                                $penalitesTraitees++;
                            }
                        }
        ...
        \Log::info("Calcul des pénalités terminés. $penalitesTraitees pénalités ont été traitées ou mises à jour.");
